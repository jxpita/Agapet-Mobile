...
export const Faq = () => {
  ...
  const [listPreguntasFrecuentes, setlistPreguntasFrecuentes] = useState([]);
    ...
    if ((buscador == "" && tipo == 1) || (tema2 == "All" && tipo == 0)) {
      ...
      if (tipo == 1) {
        fil = [
          ...new Set(
            faq.filter((c) =>
              c.pregunta.toUpperCase().includes(texto.toUpperCase())
            )
          ),
        ];
      } else {
        fil = [
          ...new Set(faq.filter((c) => c.temaid.toString().includes(tema2))),
        ];
      }
  ...
  return (
    <View style={style.fondo}>
      ...
      >
        ...
        <ImageBackground
          source={require("../../../assets/huellasfaq.jpg")}
          style={{ alignItems: "center", height: height * 0.8, width: width }}
        >
          ...
          <View style={style.backgroundContainer}>
            <SafeAreaView>
              <View>
                ...
                <View>
                  <FlatList
                    data={listPreguntasFrecuentes}
                    renderItem={({ item }) => (
                      <AccordionItem
                        title={item.pregunta}
                        bodyText={item.respuesta}
                      />
                    )}
                    keyExtractor={(item) => item.faqid.toString()}
                  />
